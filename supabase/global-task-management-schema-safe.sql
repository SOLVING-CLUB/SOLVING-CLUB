-- Global Task Management System Schema (Safe Version)
-- This schema handles existing objects gracefully with IF NOT EXISTS and DROP IF EXISTS
...
drop trigger if exists global_task_categories_set_updated_at on public.global_task_categories;
drop trigger if exists global_tasks_set_updated_at on public.global_tasks;
drop trigger if exists global_task_comments_set_updated_at on public.global_task_comments;
drop trigger if exists global_task_time_entries_set_updated_at on public.global_task_time_entries;
drop trigger if exists user_task_preferences_set_updated_at on public.user_task_preferences;
drop trigger if exists update_task_actual_hours_trigger on public.global_task_time_entries;
drop trigger if exists set_task_completed_at_trigger on public.global_tasks;
drop trigger if exists create_default_task_categories_trigger on auth.users;
